...
@property (MB_STRONG) MONActivityIndicatorView *customIndiccator;
...
-(MONActivityIndicatorView *)animatedView{
    if (!self.customIndiccator) {
        self.customIndiccator = [[MONActivityIndicatorView alloc] init];
        self.customIndiccator.center = self.center;
        [self.customIndiccator startAnimating];
    }
    return self.customIndiccator;
}
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ...
    // hud.customView = [hud animatedView];
	[view addSubview:hud];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.opacity = 0.8f;
        self.color = nil;
		self.labelFont = [UIFont fontWithName:@"OpenSans-Light" size:20.0];
        self.detailsLabelFont = [UIFont fontWithName:@"OpenSans" size:kDetailsLabelFontSize];//boldSystemFontOfSize:kDetailsLabelFontSize];
		self.xOffset = 0.0f;
		self.yOffset = 0.0f;
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
	...
	if (UIInterfaceOrientationIsLandscape(orientation)) {
		if (orientation == UIInterfaceOrientationLandscapeLeft) { radians = 0; }
		else { radians = 0; }
		// Window coordinates differ!
		self.bounds = CGRectMake(0, 0, self.bounds.size.height, self.bounds.size.width);
	} else {
		if (orientation == UIInterfaceOrientationPortraitUpsideDown) { radians = 0; }
		else { radians = 0; }
	}
